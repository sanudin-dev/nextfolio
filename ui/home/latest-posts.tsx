...
export default function LatestPosts() {
  ...
  return (
    <section id="latest-posts" className="w-full py-20 my-10 px-4 bg-muted/50 rounded-xl">
      <div className="container mx-auto max-w-4xl">
        <div className="text-center mb-10">
          ...
          <h2 className="font-bold text-2xl mb-2 text-title">Notes from the journey</h2>
          <p className="text-content">
            Things I&apos;ve learned, problems I&apos;ve solved, and ideas I want to remember.
          </p>
          <p className="text-content">
            Written for my future self, shared for anyone who finds it useful, and kept for my kids
            to read someday.
          </p>
          <Link href="/blog" className="text-primary hover:underline font-medium mt-5 inline-block">
            View all posts →
          </Link>
        </div>
          ...
          {posts.map((post) => (
            ...
            >
              <Link href={`/blog/${post.slug}`} className="group block h-full flex flex-col">
                ...
                {post.date && (
                  <time dateTime={post.date} className="text-sm text-muted-foreground mt-1 block">
                ...
                {post.description && (
                  <p className="text-content text-sm mt-2 line-clamp-2">{post.description}</p>
                )}
